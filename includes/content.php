...
const SERVICES = [
    [
        ...
        'items' => [
            ...
            /* ---- Agentic AI, six services -------------------------------
               The agentic-AI category as the market now advertises it, written
               in iThrive's own words and against what we actually deliver. The
               through-line is the site's usual one: an agent is only useful
               where its boundary is written down, so every entry names what it
               decides and what it only drafts.                               */
            [
                'slug'  => 'agentic-ai-strategy',
                'title' => 'Agentic AI Strategy & Consulting',
                'icon'  => 'compass',
                'short' => 'Find the handful of workflows where an autonomous agent pays for itself, and rule out the ones where it would only add risk.',
                'lead'  => 'Most agentic programmes fail before any code is written, because the first workflow chosen was the one that demonstrated best rather than the one that cost the most. We start by measuring the work.',
                'capabilities' => [
                    ['title' => 'Workflow cost audit',        'body' => 'We time the processes you are considering automating — volume, handling time, error rate and rework — so the business case rests on your numbers rather than a vendor benchmark.'],
                    ['title' => 'Autonomy scoring',           'body' => 'Each candidate is scored on how reversible its actions are, how well its rules are written down, and how much context lives only in someone\'s head. The last one is what usually kills a pilot.'],
                    ['title' => 'Build, buy or leave alone',  'body' => 'A written recommendation per workflow, including the ones we think you should not automate yet and why waiting is the cheaper decision.'],
                    ['title' => 'Target architecture',        'body' => 'Agent topology, model choices, memory and tool contracts drawn before implementation, so the first build is not also the first design.'],
                    ['title' => 'Risk and compliance review', 'body' => 'Where autonomy meets regulated data or money movement, we map the approval points and the evidence an auditor will ask for.'],
                    ['title' => 'A sequenced roadmap',        'body' => 'Phase one proves the mechanism on something small and reversible; later phases start only once the evaluation numbers from the previous one hold.'],
                ],
                'outcomes' => [
                    ['value' => '2-3 wks', 'label' => 'Audit to written roadmap'],
                    ['value' => '3-5',     'label' => 'Workflows scored per engagement'],
                    ['value' => '100%',    'label' => 'Recommendations with a stated assumption'],
                ],
                'stack' => ['Process mining', 'LangSmith', 'Python', 'Jupyter', 'Evaluation design'],
            ],
            [
                'slug'  => 'custom-agent-development',
                'title' => 'Custom AI Agent Development',
                'icon'  => 'bot',
                'short' => 'Single-purpose and multi-step agents built against your data, your tools and your rules — not a wrapper around a general assistant.',
                'lead'  => 'A useful agent is mostly plumbing: the tools it may call, the schema it must return, the cases it is evaluated against. The model is the least interesting part of the build, and the part that changes most often.',
                'capabilities' => [
                    ['title' => 'Tool and action design',      'body' => 'Every capability an agent has is an explicit, typed tool with its own permissions and failure behaviour. An agent that can call anything is an agent nobody can reason about.'],
                    ['title' => 'Grounding on your corpus',    'body' => 'Chunking, embedding and hybrid retrieval tuned against your own documents, with citations so any answer can be traced back to the source it came from.'],
                    ['title' => 'Structured, validated output','body' => 'Schema-constrained responses validated before they reach another system, so a malformed generation fails loudly instead of corrupting a record.'],
                    ['title' => 'Evaluation harness',          'body' => 'A golden set drawn from your real cases, run on every prompt and model change. A release that regresses on it does not ship.'],
                    ['title' => 'Prompt-injection defence',    'body' => 'Untrusted content is treated as data, never as instruction. Tool access is scoped so a hostile document cannot widen what an agent is able to do.'],
                    ['title' => 'Cost and latency ceilings',   'body' => 'Model routing, caching and token budgets per run, so unit economics survive the move from a pilot to your whole team.'],
                ],
                'outcomes' => [
                    ['value' => '4-8 wks', 'label' => 'First agent in production'],
                    ['value' => '100%',    'label' => 'Runs traced, costed and replayable'],
                    ['value' => '0',       'label' => 'Unscoped tool permissions shipped'],
                ],
                'stack' => ['Python', 'LangGraph', 'FastAPI', 'pgvector', 'Pydantic', 'Claude', 'OpenAI', 'Redis'],
            ],
            [
                'slug'  => 'multi-agent-orchestration',
                'title' => 'Multi-Agent Orchestration',
                'icon'  => 'network',
                'short' => 'Several specialised agents working one process together, with a supervisor that can stop the whole run.',
                'lead'  => 'Multi-agent is worth the extra machinery only when a process genuinely has separable roles. When it does, the hard part is not the conversation between agents — it is the state, the retries, and knowing which one failed.',
                'capabilities' => [
                    ['title' => 'Role decomposition',        'body' => 'The process is split along the seams that actually differ in skill or permission, not into as many agents as possible. Fewer agents with clearer jobs beat a crowd.'],
                    ['title' => 'Supervisor and handoff',    'body' => 'An explicit state machine owns the run: which agent holds the task, what it may hand on, and the conditions under which the supervisor halts everything.'],
                    ['title' => 'Shared memory design',      'body' => 'What the squad knows in common, what stays private to one role, and what expires — decided deliberately, because leaked context between agents is how a run goes strange.'],
                    ['title' => 'Deadlock and loop control', 'body' => 'Step ceilings, cost ceilings and cycle detection, so two agents cannot hand a task back and forth until the budget is gone.'],
                    ['title' => 'Per-agent evaluation',      'body' => 'Each role is scored on its own cases as well as on the end-to-end outcome, so a regression can be attributed rather than guessed at.'],
                    ['title' => 'Replayable runs',           'body' => 'Every run can be replayed step by step with its inputs and tool calls, which is the only practical way to debug a system that is non-deterministic by design.'],
                ],
                'outcomes' => [
                    ['value' => '6-12 wks', 'label' => 'Orchestrated squad in production'],
                    ['value' => '1',        'label' => 'Switch that halts an entire run'],
                    ['value' => '100%',     'label' => 'Runs replayable from their trace'],
                ],
                'stack' => ['LangGraph', 'Python', 'Temporal', 'PostgreSQL', 'Redis', 'OpenTelemetry'],
            ],
            [
                'slug'  => 'agentic-ai-integration',
                'title' => 'Agentic AI Integration',
                'icon'  => 'workflow',
                'short' => 'Connect agents to the ERP, CRM, ticketing and databases you already run, through one contract layer rather than six brittle scripts.',
                'lead'  => 'An agent with no reach is a chat window. The work that makes it operational is the integration layer — and that layer is where the permissions, the audit trail and the blast radius are decided.',
                'capabilities' => [
                    ['title' => 'System connectors',    'body' => 'SAP, Dynamics, Salesforce, Zoho, Tally, NetSuite, Jira, Zendesk and in-house databases, wrapped as typed tools with their own credentials and rate limits.'],
                    ['title' => 'Least-privilege access','body' => 'Each agent gets its own service identity scoped to the records it needs. No shared admin key, because a shared key makes every incident a full-estate incident.'],
                    ['title' => 'Idempotent writes',    'body' => 'Every write is safe to retry and carries a correlation id, so a timeout never becomes a duplicate invoice or a double-booked slot.'],
                    ['title' => 'Legacy adapters',      'body' => 'Where a system has no API we use file, database or UI-level automation, and put a proper contract in front of it so nothing downstream has to know.'],
                    ['title' => 'Human approval gates', 'body' => 'Actions above a threshold you set route to a queue with the agent\'s reasoning attached, and wait. The gate is part of the integration, not a setting bolted on later.'],
                    ['title' => 'Full action trail',    'body' => 'Every tool call recorded against the record it touched, so an auditor can reconstruct a decision without reading model logs.'],
                ],
                'outcomes' => [
                    ['value' => '30+',  'label' => 'Third-party systems wired to date'],
                    ['value' => '0',    'label' => 'Duplicate writes from a retry'],
                    ['value' => '100%', 'label' => 'Actions attributable to a run'],
                ],
                'stack' => ['Python', 'FastAPI', 'Celery', 'REST / SOAP / GraphQL', 'Playwright', 'PostgreSQL', 'Kafka'],
            ],
            [
                'slug'  => 'autonomous-workflow-automation',
                'title' => 'Autonomous Workflow Automation',
                'icon'  => 'zap',
                'short' => 'End-to-end processes an agent runs start to finish — intake, decision, action and follow-up — with people on the exceptions.',
                'lead'  => 'Automating a task saves minutes. Automating a workflow removes a queue. The difference is whether the agent can carry a case from arrival to resolution without a person re-entering it somewhere in the middle.',
                'capabilities' => [
                    ['title' => 'Intake and classification', 'body' => 'Email, document, form and ticket intake parsed and routed, including the scanned and inconsistent paperwork that defeats template-based tools.'],
                    ['title' => 'Decision rules in code',    'body' => 'The policy the workflow follows is versioned alongside the agent, so a change to how cases are decided is a reviewable commit rather than a prompt someone edited.'],
                    ['title' => 'Exception routing',         'body' => 'Low-confidence and out-of-policy cases go to a person with the agent\'s working shown. The measure of a good workflow is how few reach that queue, not that none do.'],
                    ['title' => 'Follow-through',            'body' => 'Chasing, reminders, status updates and closure — the unglamorous tail where most manual processes actually leak time.'],
                    ['title' => 'Shadow-mode rollout',       'body' => 'The agent runs alongside your team without acting, and its decisions are compared against theirs until the numbers justify letting it act.'],
                    ['title' => 'Throughput reporting',      'body' => 'Cases handled, deflection rate, exception rate and cost per case, reported whether or not the figures flatter us.'],
                ],
                'outcomes' => [
                    ['value' => '40%+',  'label' => 'Manual handling typically removed'],
                    ['value' => '24/7',  'label' => 'Continuous case processing'],
                    ['value' => '2 wks', 'label' => 'Typical shadow-mode period'],
                ],
                'stack' => ['Python', 'LangGraph', 'Celery', 'PostgreSQL', 'OCR / document AI', 'Grafana'],
            ],
            [
                'slug'  => 'agent-operations-support',
                'title' => 'Agent Operations, Training & Support',
                'icon'  => 'gauge',
                'short' => 'Keep deployed agents accurate as your data, policies and models change — monitoring, re-evaluation, and a rollback that works.',
                'lead'  => 'An agent is not a delivery, it is a running system. Model versions change under you, policies change around you, and accuracy drifts quietly unless something is watching for it.',
                'capabilities' => [
                    ['title' => 'Behavioural monitoring', 'body' => 'Live tracking of refusal rates, tool-call failures, latency, cost per run and output-schema violations, with alerts tuned to page a person only when one is needed.'],
                    ['title' => 'Drift detection',        'body' => 'The golden set is re-run on a schedule and on every upstream model change, so accuracy loss is caught by us rather than reported by your customer.'],
                    ['title' => 'Model migration',        'body' => 'When a provider deprecates a version we re-evaluate, re-tune and cut over against the same test set, so the change is measured rather than hoped through.'],
                    ['title' => 'Knowledge refresh',      'body' => 'Re-indexing as your documents and policies change, including removal — an agent quoting a withdrawn policy is worse than one that says it does not know.'],
                    ['title' => 'Incident response',      'body' => 'A defined stop, a defined rollback and a written post-incident note. Autonomy you cannot halt mid-run is exposure rather than capability.'],
                    ['title' => 'Team enablement',        'body' => 'Your engineers pair with ours through the build and hold the runbook at the end. A permanent dependency on us is not the goal.'],
                ],
                'outcomes' => [
                    ['value' => '99.9%',  'label' => 'Monitored agent availability'],
                    ['value' => '1 cmd',  'label' => 'Halt and roll back a squad'],
                    ['value' => 'Weekly', 'label' => 'Golden-set re-evaluation cadence'],
                ],
                'stack' => ['LangSmith', 'OpenTelemetry', 'Grafana', 'Python', 'GitHub Actions', 'PostgreSQL'],
            ],
            [
                'slug'  => 'gen-ai-development',
                'title' => 'Generative AI Development',
                'icon'  => 'sparkles',
                'short' => 'Generation put to work on a specific job — drafting, summarising, extracting, translating — with the output checked before anything downstream trusts it.',
                'lead'  => 'Generative features are easy to demonstrate and hard to keep honest. The engineering is in the grounding, the validation and the evaluation set, not in the prompt.',
                'capabilities' => [
                    ['title' => 'Drafting and summarisation', 'body' => 'Long documents, call transcripts and ticket threads reduced to the summary a particular role actually needs, in the format the next system expects.'],
                    ['title' => 'Structured extraction',      'body' => 'Fields pulled from unstructured paperwork into a validated schema, with a confidence score and a review queue for anything below it.'],
                    ['title' => 'Grounded generation',        'body' => 'Output tied to your own sources with citation, so a claim can be checked rather than taken on trust.'],
                    ['title' => 'Brand and tone control',     'body' => 'Style enforced through examples and post-checks, so generated copy reads like your organisation rather than like a model.'],
                    ['title' => 'Evaluation before release',  'body' => 'A golden set of your real cases scored on every prompt or model change. A regression blocks the release rather than reaching a customer.'],
                    ['title' => 'Cost and latency control',   'body' => 'Cheap work routed to small models, repeated work cached, and tokens capped per call so the economics hold at full volume.'],
                ],
                'outcomes' => [
                    ['value' => '3-6 wks', 'label' => 'First generative feature live'],
                    ['value' => '100%',    'label' => 'Outputs schema-validated'],
                    ['value' => '0',       'label' => 'Ungrounded claims shipped knowingly'],
                ],
                'stack' => ['Python', 'Claude', 'OpenAI', 'FastAPI', 'pgvector', 'Pydantic', 'LangSmith'],
            ],
            [
                'slug'  => 'ai-chatbot-development',
                'title' => 'AI Chatbot Development',
                'icon'  => 'message',
                'short' => 'Assistants on your website, app, WhatsApp or helpdesk that answer from your own content and hand over cleanly when they should not answer.',
                'lead'  => 'A chatbot is judged on the questions it gets wrong, not the ones it gets right. So the design starts with what it must refuse, and with what happens next when it does.',
                'capabilities' => [
                    ['title' => 'Answering from your content', 'body' => 'Retrieval over your documentation, policies and product data, with the source shown beside the answer.'],
                    ['title' => 'Escalation that works',       'body' => 'A written handover to a person with the conversation attached, triggered by low confidence, sentiment, or an explicit request.'],
                    ['title' => 'Channel deployment',          'body' => 'Web widget, in-app, WhatsApp Business and helpdesk integration from one backend, so answers cannot drift between channels.'],
                    ['title' => 'Transactional actions',       'body' => 'Order status, booking changes and ticket creation as typed tools with their own permissions, not as free text the bot improvises.'],
                    ['title' => 'Multilingual handling',       'body' => 'Answering in the language asked, including Indian languages, with the source content maintained in one place.'],
                    ['title' => 'Containment reporting',       'body' => 'Deflection rate, escalation reasons and unanswered questions reported weekly, because the unanswered list is the roadmap.'],
                ],
                'outcomes' => [
                    ['value' => '3-5 wks', 'label' => 'Assistant in production'],
                    ['value' => '60%+',    'label' => 'Typical first-line deflection'],
                    ['value' => '100%',    'label' => 'Answers carrying a source'],
                ],
                'stack' => ['Python', 'FastAPI', 'pgvector', 'Claude', 'WhatsApp Business API', 'Redis', 'Zendesk / Freshdesk APIs'],
            ],
            [
                'slug'  => 'rpa-development',
                'title' => 'RPA Development',
                'icon'  => 'cpu',
                'short' => 'Robotic process automation for the systems that have no API — the bridge that retires a repetitive job without a vendor migration.',
                'lead'  => 'RPA has a reputation for breaking whenever a screen changes, and it earns that reputation when robots are recorded rather than engineered. Built properly it is the cheapest way to automate a system nobody will ever give you an API for.',
                'capabilities' => [
                    ['title' => 'Process capture',         'body' => 'The task documented as it is actually performed, including the exceptions people handle without noticing they are exceptions.'],
                    ['title' => 'Resilient selectors',     'body' => 'Robots driven by stable identifiers rather than screen coordinates, so a layout change is a fix rather than a rebuild.'],
                    ['title' => 'Attended and unattended', 'body' => 'Robots that assist a person at their desk, and robots that run on a schedule against a queue, with different controls for each.'],
                    ['title' => 'Exception handling',      'body' => 'Every failure path defined: retry, park for review, or stop. A robot that silently skips a record is worse than no robot.'],
                    ['title' => 'Agent-assisted steps',    'body' => 'Where a step needs judgement — reading a scanned form, deciding a category — a model does that part and the robot does the rest.'],
                    ['title' => 'Audit and reporting',     'body' => 'Every run logged with what it touched and what it changed, so the automation is auditable like any other actor.'],
                ],
                'outcomes' => [
                    ['value' => '2-6 wks', 'label' => 'First robot in production'],
                    ['value' => '70%+',    'label' => 'Typical hands-on time removed'],
                    ['value' => '100%',    'label' => 'Runs logged and reviewable'],
                ],
                'stack' => ['Python', 'Playwright', 'Selenium', 'UiPath', 'Power Automate', 'Celery', 'PostgreSQL'],
            ],
            [
                'slug'  => 'rag-development',
                'title' => 'RAG Development',
                'icon'  => 'database',
                'short' => 'Retrieval-augmented generation over your own corpus, built so answers cite their source and stale content cannot quietly survive.',
                'lead'  => 'Most disappointing RAG systems are not a model problem. They are a retrieval problem — the right passage was never fetched, so the model filled the gap itself.',
                'capabilities' => [
                    ['title' => 'Ingestion and chunking',   'body' => 'PDFs, scans, spreadsheets, wikis and ticket history parsed into chunks that preserve the structure an answer depends on.'],
                    ['title' => 'Hybrid retrieval',         'body' => 'Vector search combined with keyword search and reranking, because pure embedding search misses exact identifiers, codes and names.'],
                    ['title' => 'Citation and traceability','body' => 'Every answer carries the passages it used, so a reader can verify it without trusting the summary.'],
                    ['title' => 'Freshness and removal',    'body' => 'Re-indexing on change including deletions — a system quoting a withdrawn policy is worse than one that says it does not know.'],
                    ['title' => 'Access-aware retrieval',   'body' => 'Permissions enforced at retrieval, so the assistant cannot surface a document the person asking is not allowed to read.'],
                    ['title' => 'Retrieval evaluation',     'body' => 'Recall measured on a labelled question set separately from answer quality, so a failure can be attributed to the right stage.'],
                ],
                'outcomes' => [
                    ['value' => '3-6 wks', 'label' => 'Grounded assistant in production'],
                    ['value' => '100%',    'label' => 'Answers carrying citations'],
                    ['value' => '0',       'label' => 'Documents served past permission'],
                ],
                'stack' => ['Python', 'pgvector', 'OpenSearch', 'LangGraph', 'Unstructured', 'Cohere Rerank', 'FastAPI'],
            ],
            [
                'slug'  => 'computer-vision-development',
                'title' => 'Computer Vision Development',
                'icon'  => 'search',
                'short' => 'Detection, classification, reading and counting from images and video — on the line, in the field, or over documents.',
                'lead'  => 'Vision projects succeed or fail on the data, not the architecture. A model trained on clean sample images and deployed against a dirty lens in poor light is the usual way this goes wrong.',
                'capabilities' => [
                    ['title' => 'Data collection and labelling', 'body' => 'A labelled set built from your own conditions — your lighting, your angles, your failure cases — because that is what the model will meet.'],
                    ['title' => 'Detection and classification',  'body' => 'Defect spotting, presence and absence checks, counting, and sorting, tuned to the error you can least afford.'],
                    ['title' => 'Document and OCR pipelines',    'body' => 'Reading printed and handwritten forms, invoices and IDs, including the scanned pages template tools cannot handle.'],
                    ['title' => 'Edge and on-device inference',  'body' => 'Models that run where the camera is, so a network outage does not stop the line and video never has to leave the site.'],
                    ['title' => 'Human review loop',             'body' => 'Low-confidence frames routed to a person, and their correction fed back into the next training round rather than discarded.'],
                    ['title' => 'Drift monitoring',              'body' => 'Accuracy tracked against a held-out set over time, because a changed camera, lens or product finish degrades a model quietly.'],
                ],
                'outcomes' => [
                    ['value' => '6-12 wks', 'label' => 'Pilot to production line'],
                    ['value' => '95%+',     'label' => 'Typical accepted accuracy target'],
                    ['value' => 'On-site',  'label' => 'Inference without sending video out'],
                ],
                'stack' => ['Python', 'PyTorch', 'OpenCV', 'YOLO', 'ONNX Runtime', 'NVIDIA Jetson', 'FastAPI'],
            ],
            [
                'slug'  => 'ai-copilot-development',
                'title' => 'AI Copilot Development',
                'icon'  => 'lightbulb',
                'short' => 'An assistant embedded inside your product or internal tool that helps the person using it, rather than replacing them.',
                'lead'  => 'A copilot lives inside a screen someone already works in, which makes context the whole problem: it should know what the user is looking at without being told, and never act without being asked.',
                'capabilities' => [
                    ['title' => 'In-context awareness',   'body' => 'The copilot receives the record, filters and selection currently on screen, so the user does not have to describe what they are looking at.'],
                    ['title' => 'Suggest, do not act',    'body' => 'Proposals presented for acceptance by default. Anything that writes to a system is an explicit confirmation, not an inference.'],
                    ['title' => 'Domain grounding',       'body' => 'Answers drawn from your product documentation, data model and policies, so guidance matches the system rather than the internet.'],
                    ['title' => 'Inline UI patterns',     'body' => 'Side panel, command palette or inline completion, chosen for how the screen is actually used rather than for what demonstrates well.'],
                    ['title' => 'Feedback capture',       'body' => 'Accept, edit and reject recorded per suggestion, which is both the quality signal and the training set for the next iteration.'],
                    ['title' => 'Per-seat cost control',  'body' => 'Budgets and model routing per user and per feature, so a copilot rolled out to everyone does not surprise you at the end of the month.'],
                ],
                'outcomes' => [
                    ['value' => '4-8 wks', 'label' => 'Copilot inside an existing product'],
                    ['value' => '100%',    'label' => 'Write actions user-confirmed'],
                    ['value' => 'Per-seat','label' => 'Cost visible before rollout'],
                ],
                'stack' => ['TypeScript', 'React', 'Python', 'FastAPI', 'Claude', 'pgvector', 'WebSockets'],
            ],
            [
                'slug'  => 'hire-agentic-ai-developers',
                'title' => 'Hire Agentic AI Developers',
                'icon'  => 'users',
                'short' => 'Named AI engineers embedded in your team, working in your repository and your stand-up, without the hiring cycle.',
                'lead'  => 'Agentic engineering is a young speciality and a slow hire. This is the same people who build our own agent work, working on your roadmap under your direction instead of ours.',
                'capabilities' => [
                    ['title' => 'Named, not pooled',        'body' => 'You get specific engineers with names and history, not a rotating bench. If someone changes, you agree it beforehand.'],
                    ['title' => 'Your tools, your process', 'body' => 'They work in your repository, your tracker and your review process, and attend your stand-up. Nothing routes through a separate account.'],
                    ['title' => 'Agentic specialisms',      'body' => 'Retrieval, orchestration, evaluation, tool design and inference cost — the skills that are hardest to hire for individually.'],
                    ['title' => 'Evaluation discipline',    'body' => 'They bring the golden-set habit with them, so quality is measured from the first week rather than retrofitted before launch.'],
                    ['title' => 'Knowledge transfer',       'body' => 'Pairing and written handover throughout, on the assumption that you will eventually run this without us.'],
                    ['title' => 'Monthly, not annual',      'body' => 'Scale up or down each month. A team you cannot resize is a hire with extra steps.'],
                ],
                'outcomes' => [
                    ['value' => '1-2 wks', 'label' => 'From brief to engineers starting'],
                    ['value' => 'Monthly', 'label' => 'Commitment, resizable'],
                    ['value' => '100%',    'label' => 'Work in your own repository'],
                ],
                'stack' => ['Python', 'LangGraph', 'TypeScript', 'PostgreSQL', 'AWS', 'Docker', 'GitHub'],
            ],
            [
                'slug'  => 'ai-consulting',
                'title' => 'AI Consulting',
                'icon'  => 'target',
                'short' => 'Enterprise-wide AI strategy — where AI belongs in your organisation, what it will cost, what it will not fix, and the order to do it in.',
                'lead'  => 'This is the engagement that sits above any single build. Agentic AI Strategy picks the workflows for an agent; this one decides whether AI is the right instrument at all, across a portfolio, and what has to be true first.',
                'capabilities' => [
                    ['title' => 'AI readiness assessment', 'body' => 'An honest look at the three things that decide whether AI works here: the state of your data, the clarity of your processes, and whether anyone owns the outcome.'],
                    ['title' => 'Opportunity portfolio',   'body' => 'Candidate use cases across departments, each sized on value against effort, and explicitly ranked — including the ones we would not fund this year.'],
                    ['title' => 'Build, buy or wait',      'body' => 'For each candidate: build it, buy a product that already does it, or wait. Recommending a licence over a project is a normal outcome of this work.'],
                    ['title' => 'Model and vendor review', 'body' => 'Independent comparison on your own tasks rather than public benchmarks, including where an open-weight model on your own infrastructure is the cheaper answer.'],
                    ['title' => 'Governance and policy',   'body' => 'Approval thresholds, data handling, retention and the record you will need when a regulator or customer asks how a decision was made.'],
                    ['title' => 'Team and capability plan','body' => 'What to hire, what to train, and what to outsource, so the programme survives the departure of any one person.'],
                ],
                'outcomes' => [
                    ['value' => '3-5 wks', 'label' => 'Assessment to board-ready plan'],
                    ['value' => '10-20',   'label' => 'Use cases assessed and ranked'],
                    ['value' => '100%',    'label' => 'Recommendations with costs attached'],
                ],
                'stack' => ['Discovery workshops', 'Data audit', 'Benchmarking on your tasks', 'Python', 'Cost modelling'],
            ],
            [
                'slug'  => 'ai-integration',
                'title' => 'AI Integration Services',
                'icon'  => 'layers',
                'short' => 'Put models into the systems your business already runs on — ERP, CRM, data platform and internal tools — as reliable, monitored infrastructure.',
                'lead'  => 'Distinct from the two services either side of it: AI Enablement adds intelligence to your own product, Agentic AI Integration gives agents permissioned reach, and this is the plumbing that serves models to the enterprise estate underneath both.',
                'capabilities' => [
                    ['title' => 'Model serving',        'body' => 'Inference endpoints with autoscaling, timeouts, retries and a fallback path, so a provider outage degrades a feature rather than an application.'],
                    ['title' => 'Enterprise connectors','body' => 'SAP, Dynamics, Salesforce, Zoho, Tally and in-house systems wired to AI services through one contract layer rather than per-team scripts.'],
                    ['title' => 'Data pipelines',       'body' => 'The extraction, cleaning and embedding work that turns operational records into something a model can actually use, running on a schedule.'],
                    ['title' => 'Private and on-prem',  'body' => 'Open-weight models hosted in your own environment where residency, cost at volume, or contract terms rule out a public API.'],
                    ['title' => 'Gateway and governance','body' => 'One route for every AI call, with per-team keys, quotas, logging and cost attribution, so spend and usage are answerable by department.'],
                    ['title' => 'Monitoring and rollback','body' => 'Latency, error and cost dashboards per integration, and a documented way back to the previous behaviour when a model changes under you.'],
                ],
                'outcomes' => [
                    ['value' => '4-8 wks', 'label' => 'First integration in production'],
                    ['value' => '1',       'label' => 'Gateway every AI call routes through'],
                    ['value' => '100%',    'label' => 'Spend attributable by team'],
                ],
                'stack' => ['Python', 'FastAPI', 'Kafka', 'Airflow', 'vLLM', 'Kubernetes', 'PostgreSQL', 'Grafana'],
            ],
            [
                'slug'  => 'ai-agent-solutions',
                'title' => 'AI Agent Solutions',
                'icon'  => 'rocket',
                'short' => 'Ready-to-deploy agents for the jobs most businesses share — support, sales follow-up, document handling, reporting — configured to your data in days.',
                'lead'  => 'The counterpart to Custom AI Agent Development. Not every agent needs designing from nothing: the common jobs have been built enough times that the sensible route is a proven archetype pointed at your systems, with a path to a bespoke build when you outgrow it.',
                'capabilities' => [
                    ['title' => 'A catalogue of archetypes',  'body' => 'Support triage, sales follow-up, document extraction, reconciliation, reporting and scheduling — each one already carrying its tools, prompts and evaluation set.'],
                    ['title' => 'Configured, not generic',    'body' => 'An archetype is a starting point. It is pointed at your data, your terminology and your approval thresholds before it answers anyone.'],
                    ['title' => 'Live in days',               'body' => 'Because the build is done, the work is connection and configuration. Most archetypes are running against your systems inside a week.'],
                    ['title' => 'The same guardrails',        'body' => 'Typed tools, scoped permissions, schema validation and a full action trail — identical to a bespoke build. Faster to deploy does not mean looser.'],
                    ['title' => 'Shadow mode first',          'body' => 'It runs alongside your team without acting, and its decisions are compared against theirs until the numbers justify letting it act.'],
                    ['title' => 'An upgrade path',            'body' => 'When an archetype stops fitting, it becomes the specification for a custom agent rather than something to throw away and restart.'],
                ],
                'outcomes' => [
                    ['value' => '3-7 days', 'label' => 'Archetype configured and live'],
                    ['value' => '100%',     'label' => 'Same guardrails as a custom build'],
                    ['value' => 'Any time', 'label' => 'Upgrade path to a bespoke agent'],
                ],
                'stack' => ['Python', 'LangGraph', 'FastAPI', 'pgvector', 'Claude', 'Redis', 'PostgreSQL'],
            ],
        ],
    ],
    [
...
const CASE_STUDIES = [
    ...
    /* The three built websites below are written only from what their live
       sites show. None has production outcome data behind it, so the metrics
       are things a visitor can verify on the site itself rather than claimed
    ...
    [
        ...
        'url'       => 'https://maduragrandeur.com/',
        'industry'  => 'Hospitality — Hotel & Banquets',
        'icon'      => 'building',
        'accent'    => '#D0504F',
        'logo'      => 'madura-grandeur.png',
        'mock'      => 'desktop',
        'featured'  => false,
        'categories'=> ['web-platforms'],
        'summary'   => 'A hotel website for Madura Grandeur in Madurai, with published room tariffs, a direct booking request form, a filterable gallery, careers and contact in one place.',
        'challenge' => 'Madura Grandeur needed guests to see its rooms and prices and ask for a stay directly, without going through an online travel agency. The site had to carry three room categories with single and double tariffs, hotel policies, recruitment and contact details, and still put a booking action within reach of every page.',
        'solution'  => 'We built a responsive hotel website with a booking request form available from every page: guest details, check-in and check-out dates, adults and children, room type and acceptance of the room, payment and hotel policies, confirmed on screen once submitted. Room pages list tariffs and amenities, the gallery filters by area of the property, a careers section lists open roles with an application form, and the contact page carries an enquiry form, map, working hours and FAQs.',
        'value'     => 'Guests can go from browsing rooms to a booking request with the hotel itself, and the same site handles recruitment and general enquiries.',
        'metrics'   => [
            ['value' => '3',        'label' => 'Room categories with published tariffs'],
            ['value' => '6',        'label' => 'Open roles with online applications'],
            ['value' => 'Every page','label' => 'Booking request one click away'],
        ],
        'features'  => [
            ['icon' => 'calendar', 'title' => 'Booking request form', 'body' => 'Guest details, stay dates, party size and room type, with room, payment and hotel policies accepted before submission and an on-screen confirmation.'],
            ['icon' => 'building', 'title' => 'Rooms and tariffs',    'body' => 'Standard, Deluxe and Master Family Suite, each with single, double or occupancy pricing and its amenities listed.'],
            ['icon' => 'search',   'title' => 'Filterable gallery',   'body' => 'Rooms, exterior, entrance, night view, lobby, hallways and bathrooms, filterable by area of the property.'],
            ['icon' => 'users',    'title' => 'Careers',              'body' => 'Open positions from front desk to executive chef, each with an Apply Now route into an application form.'],
            ['icon' => 'pin',      'title' => 'Contact and FAQs',     'body' => 'An enquiry form with a subject selector, embedded map, working hours and answers on check-in times, pickup, parking, cancellation and pets.'],
            ['icon' => 'message',  'title' => 'WhatsApp and call',    'body' => 'A floating WhatsApp button and click-to-call numbers so guests on a phone can reach reservations in one tap.'],
        ],
        'stack'     => ['HTML', 'Tailwind CSS', 'JavaScript', 'Swiper', 'Google Maps', 'Netlify'],
        'screens'   => ['Home and booking', 'Rooms and tariffs', 'Gallery', 'Careers'],
        'badges'    => ['web'],
    ],
    [
        ...
        'url'       => 'https://bharanibeautyclinic.com/',
        'industry'  => 'Beauty & Wellness',
        'icon'      => 'sparkles',
        'accent'    => '#E27AA2',
        'logo'      => 'bharani-beauty-clinic.png',
        'mock'      => 'desktop',
        'featured'  => false,
        'categories'=> ['web-platforms'],
        'summary'   => 'A website for Bharani Beauty Clinic in Gobichettipalayam presenting bridal makeup, beauty and spa services, the parlour\'s history since 1988, client reviews and a way to get in touch.',
        'challenge' => 'Bharani Beauty Clinic has served Gobichettipalayam since 1988 and needed a web presence that carries that reputation: a clear menu of services, bridal work shown in photographs, reviews from long-standing clients, and an easy way to make contact from a phone.',
        'solution'  => 'We built the site on Hostinger Website Builder, so the clinic can update its services and photographs itself. It covers the founder\'s story, a service menu running from wedding makeup and saree draping to facials, waxing and spa treatments, answers to common questions, a photo gallery, customer reviews, an enquiry form, and the clinic\'s location and opening hours.',
        'value'     => 'Decades of work are presented in one place that clients can browse and contact from their phones, and the clinic can keep it current without a developer.',
        'metrics'   => [
            ['value' => 'Since 1988','label' => 'Heritage told on the About page'],
            ['value' => '7 days',    'label' => 'Opening hours published online'],
            ['value' => 'Self-edit', 'label' => 'Services and photos updated in-house'],
        ],
        'features'  => [
            ['icon' => 'sparkles', 'title' => 'Service menu',      'body' => 'Wedding makeup, saree draping and children\'s makeup alongside facials, eyebrows, manicure, pedicure, waxing, mehndi, massage and spa.'],
            ['icon' => 'heart',    'title' => 'Bridal portfolio',  'body' => 'Bridal and occasion looks shown through full-width photography across the home page and gallery.'],
            ['icon' => 'star',     'title' => 'Customer reviews',  'body' => 'Five-star testimonials from clients, including families who have come to the parlour across generations.'],
            ['icon' => 'users',    'title' => 'Founder\'s story',  'body' => 'The parlour\'s history since 1988 and the founder\'s vision, told on its own About page.'],
            ['icon' => 'message',  'title' => 'Enquiry form',      'body' => 'Name, phone and message, alongside email, phone numbers and an Instagram link for clients who prefer those.'],
            ['icon' => 'clock',    'title' => 'Location and hours','body' => 'The clinic\'s address in Gobichettipalayam and its opening hours for weekdays and Sundays.'],
        ],
        'stack'     => ['Hostinger Website Builder'],
        'screens'   => ['Home', 'Services and FAQs', 'Gallery', 'Contact and hours'],
        'badges'    => ['web'],
    ],
    [
        'slug'      => 'aruvanaa',
        ...
        'url'       => 'https://aruvanaa.com/',
        'industry'  => 'Agri-Commodity Export & International Trade',
        'icon'      => 'globe',
        'accent'    => '#4FA35A',
        'logo'      => 'aruvanaa.png',
        'mock'      => 'desktop',
        'featured'  => false,
        'categories'=> ['web-platforms'],
        'summary'   => 'A corporate website for Aruvanaa Private Limited, which sources agricultural commodities from India for buyers in the UAE, covering its commodities, integrated supply-chain services and compliance credentials.',
        'challenge' => 'Aruvanaa trades Indian agricultural produce into the UAE and needed a site that establishes credibility with overseas buyers: what it sources, how it runs the supply chain end to end, which registrations and certifications it holds, and how to reach its offices in India and the UAE.',
        'solution'  => 'We built a React and Vite website with a commodity catalogue, an integrated-services timeline that walks through procurement, quality inspection, export packaging, documentation, freight forwarding, customs clearance and last-mile delivery in the UAE, a trust and compliance page with its registrations, a downloadable brochure, and India and UAE contact details on every page.',
        'value'     => 'Buyers in both markets can assess Aruvanaa\'s range, process and credentials before the first conversation.',
        'metrics'   => [
            ['value' => '2', 'label' => 'Offices: India and the UAE'],
            ['value' => '7', 'label' => 'Supply-chain stages explained'],
            ['value' => '4', 'label' => 'Registrations shown, from FSSAI to UAE'],
        ],
        'features'  => [
            ['icon' => 'package',  'title' => 'Commodity catalogue',  'body' => 'Vegetables and fruits, grains and legumes, dates, dairy, ghee and oils, eggs, potatoes, snacks and soft drinks.'],
            ['icon' => 'workflow', 'title' => 'Integrated services',  'body' => 'A step-by-step timeline from sourcing and quality inspection to customs clearance and last-mile delivery in Dubai.'],
            ['icon' => 'shield',   'title' => 'Trust and compliance', 'body' => 'FSSAI certification, Import Export Code, UAE Municipality registration and phytosanitary certification, presented together.'],
            ['icon' => 'globe',    'title' => 'Two-market contact',   'body' => 'India and UAE phone numbers in the header and separate India and UAE office details in the footer of every page.'],
            ['icon' => 'edit',     'title' => 'Brochure download',    'body' => 'A downloadable company brochure available from the header and footer for buyers to share internally.'],
            ['icon' => 'check',    'title' => 'Why choose us',        'body' => 'End-to-end supply chain, sourcing network, quality and compliance, pricing, UAE delivery and private labelling support.'],
        ],
        'stack'     => ['React', 'Vite', 'JavaScript', 'LiteSpeed'],
        'screens'   => ['Home', 'Commodities', 'Integrated services', 'Trust and compliance'],
        'badges'    => ['web'],
    ],
    [
        'slug'      => 'aeron',
        'client'    => 'AERON',
        'title'     => 'AERON — Drone Training & Pilot Logbook Platform',
        'headline'  => 'Training schools, pilot logbooks and 3D flight replay on one platform',
        'url'       => 'https://aeron-app.vercel.app/',
        'industry'  => 'Drone Training & Aviation Compliance',
        'icon'      => 'drone',
        'accent'    => '#3B7BFF',
        'logo'      => 'aeron.png',
        'mock'      => 'desktop',
        'featured'  => false,
        'categories'=> ['web-platforms'],
        'summary'   => 'A training-management platform for DGCA-authorised drone schools (RPTOs), with logbooks, 3D flight replay and a browser simulator for every pilot they train.',
        'challenge' => 'Drone training schools run admissions, batches, tests and certification for DGCA-authorised courses, and every remote pilot keeps a pilot logbook, a flight log for each drone, a battery log and a maintenance log. Kept by hand, that is a stack of forms after every flight and a record package assembled at the end of every course.',
        'solution'  => 'We built AERON as one platform with separate workspaces for platform admins, training schools, students and pilots behind a single sign-in. Schools manage admissions and documents, auto-scheduled batches, tests from online theory to printed OMR sheets, automatically numbered certificates with a ZIP record package, and a CRM with GST fees and receipts. Pilots upload ArduPilot, PX4 or CSV flight logs and get a 3D replay from five camera angles with flight-health checks and PID tuning hints, while their four logbooks fill themselves from those flights. A browser simulator with real flight modes, scored drills and voice coaching lets trainees practise before they fly.',
        'value'     => 'A trainee\'s record is complete the day they finish, and a pilot\'s logbooks stay current without retyping — exportable as PDF or Excel whenever an inspector asks.',
        'metrics'   => [
            ['value' => '4',  'label' => 'Logbooks filled from flight logs'],
            ['value' => '5',  'label' => 'Camera angles in 3D flight replay'],
            ['value' => '11', 'label' => 'Languages for simulator voice coaching'],
        ],
        'features'  => [
            ['icon' => 'drone',    'title' => '3D flight replay',        'body' => 'ArduPilot .bin and .log, PX4 .ulg and CSV exports rebuilt in 3D, viewed from follow, chase, top, side and FPV cameras, with graphs, health checks, PID hints and a printable report.'],
            ['icon' => 'refresh',  'title' => 'Self-filling logbooks',   'body' => 'Pilot, flight, battery and maintenance logbooks written from uploaded flight logs and school-signed sessions, printable or exported to Excel.'],
            ['icon' => 'monitor',  'title' => 'Browser simulator',       'body' => 'Loiter, AltHold and Stabilize modes, arming and failsafes, scored drills from hover-and-land to an agri spray field and FPV gates, on a keyboard or USB transmitter.'],
            ['icon' => 'calendar', 'title' => 'School management',       'body' => 'Admissions and document checks, one-click batch scheduling across ground classes, simulator and flying slots, and per-trainee progress.'],
            ['icon' => 'check',    'title' => 'Tests and certificates',  'body' => 'Online theory, simulator and practical tests or OMR sheets, then certificates signed by the RPA trainer, numbered automatically with a ZIP record package.'],
            ['icon' => 'users',    'title' => 'Roles and portals',       'body' => 'Platform admin, RPTO staff as Accountable Manager, Admin, Instructor or Sales, students and pilots, each landing in their own workspace from one sign-in.'],
        ],
        'stack'     => ['three.js', 'JavaScript', 'PWA', 'Vercel'],
        'screens'   => ['3D flight replay', 'Self-filling logbooks', 'Practice simulator', 'RPTO registration'],
        'badges'    => ['web'],
    ],
];
